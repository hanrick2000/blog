...
#
pandoc -f markdown+emoji -t html5  --templat=$HOME/.pandoc/templates/default.html -M updatedDate="Updated at $(date "+%B %e, %Y")"  -s --highlight-style "${hs}"  --toc  --toc-depth=4 "${md}" | while IFS= read -r line; do
  # printf '%s\n' "$line"
  i=$((i+1))
